...
func _physics_process(delta):
	...
	if(can_descend):
		if Input.is_action_just_pressed("down"):
			position.y += 3
			#current_one_way.collision_layer = 0
			#current_one_way.collision_mask = 0
			#current_one_way.get_parent().get_node("OneWayTimer").start(1)
			can_descend = false
			grabbed_ladder.emit()
...
func _on_interact_area_entered(area):
	...
	if area.is_in_group("one_way_platform"):
		can_descend = true
		current_one_way = area.get_parent().get_node("Platform")

func _on_interact_area_exited(area):
	print("Interact area exited")
	if area.is_in_group("ladder"):
